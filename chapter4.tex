...
This is an example of the approach that we took for this project. Before
...
We performed the default active scan against \url{https://linkero.ie}. This
...
\begin{description}
  ...
  decorator on the web-app page that we do not want the user to cache. This has
  ...
  clients' cookies to append CSRF tokens to AJAX requests. We decided to keep
...
\textbf{Cookie No HttpOnly Flag} issue, which is something we expected and
decided to accept.
...
\texttt{settings.py} file, as such it needs to be replaced by a new one directly
in the file stored on the actual virtual machine serving the site.
