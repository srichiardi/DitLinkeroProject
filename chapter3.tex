...
explanation, because it deviates from the standard three-tier architacture of
many websites.
...
\begin{itemize}
  ...
  \item every user would be alligned on the latest version, since any new
...
controller (who owns the data) can control the numer of requests submitted by
...
The other issue emering from the web-application approach, related to the time
necessary to comple a report. The eBay API results from a text search are
paginated, and each page returns a maximus of 100 items. There are a total of 22
country sites where the same seach can be repeated. That means that a text
...
tab open untill the request is completed.
...
decoupled components \cite{}. In Likero the broker is the part of the
